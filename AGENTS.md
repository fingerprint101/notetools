# Repository Guidelines

## Project Structure & Module Organization
`main.go` is the CLI entrypoint. User-facing commands live in `cmd/` and should stay thin: argument parsing, file I/O, and progress messages belong there. Core behavior lives in `internal/` packages such as `clean`, `merge`, `plan`, `explain`, `pdf`, and provider adapters in `internal/opencode`, `internal/claude`, and `internal/codex`. Sample inputs and expected outputs for manual checks live in `tests/`.

## Build, Test, and Development Commands
Use `make build` to compile `nt` in the repo root. Use `make test` or `go test ./...` to run the Go test suite. Use `make install` to install the binary to `/usr/local/bin/nt`. For quick manual validation, run commands directly, for example: `go run . preview tests/C-06-geographically-distributed-applications-claude.md:1-20` or `go run . plan source.md target.md`.

## Coding Style & Naming Conventions
This is a Go 1.21+ project. Format all edited Go files with `gofmt -w`. Follow standard Go naming: exported identifiers use `CamelCase`, unexported helpers use `camelCase`, and package names stay short and lowercase. Keep command packages focused on orchestration and push prompt logic, parsing, and provider-specific behavior into `internal/`. Prefer explicit errors with context such as `fmt.Errorf("sectioning failed: %w", err)`.

## Testing Guidelines
Add table-driven `*_test.go` files next to the package they cover. Prefer unit tests for parsing, prompt shaping, and mapping logic; avoid depending on live LLM CLIs in automated tests. Use the files in `tests/` for manual end-to-end checks when changing prompt behavior or output formatting. Before opening a PR, run `go test ./...` and at least one relevant manual CLI flow.

## Commit & Pull Request Guidelines
Recent commits use short, imperative summaries such as `merge planning command` and `improved merge command with diff view`. Keep commit subjects concise and descriptive. PRs should explain the user-visible change, note any prompt or provider behavior changes, include the verification commands you ran, and attach sample output when formatting or generated Markdown changes.

## Configuration & Provider Notes
Runtime configuration is stored in `~/.config/nt/config.json`. Do not commit secrets, API keys, or machine-specific provider settings. When changing provider adapters, preserve consistent behavior across `opencode`, `claude`, and `codex`, especially for JSON output and image handling.
